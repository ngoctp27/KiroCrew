...
| `slack/transport.py` | `SlackTransport` — Slack as a concrete `MessagingTransport`; the gateway constructs it with the resolved prompt roster, and Socket Mode uses its authorization snapshot before normal dispatch |
| `slack/transport_dispatch.py` | The new-path turn dispatcher `events.py` invokes after the live `SlackTransport.authorize` decision when `messaging.use_transport` is on: `handle_message_transport` builds a `TurnDriver` and `SlackRenderer` over the existing Slack client |
...
- **`SlackTransport`** (`slack/transport.py`): wraps `SlackClientOps` in the neutral `MessagingTransport` contract (dependency direction `slack → messaging`; the `messaging` package never imports Slack). `authorize()` is **roster-based and deny-by-default** — the gateway constructs it with the immutable owner-plus-member prompt roster, and every rejection is SEL-audited (`operation="slack_transport.authorize"`, `outcome="denied"`), including empty/missing `user_id`. The Socket Mode route consults this same live snapshot before native or `handle_message_transport` dispatch; activation, queue, attachment, and rich-streaming behavior remains in the gateway route.
...
3. `handler.py` posts a native Block Kit card with Approve / Trust / Reject. Trust is scoped to the card's session (not to whoever clicks it) and is offered in both DMs and configured channel threads; only an owner/co-admin can press it, same as Approve/Reject. Linked dashboard mirrors intentionally omit Trust and, like the native path, are owner/co-admin-only (not any roster member — see step 6). Gateway background cards preserve the previous DM-only Trust policy (`allow_trust=is_dm`) so channel-thread cron/taskrunner/subagent prompts remain Approve/Reject-only.
...
6. Approval button message is replaced with outcome text; a separate threaded message names who resolved it (`🔐 <label> — <@user>`, e.g. `✅ Approved`/`🚫 Rejected`/`🤝 Trusted (session trust ON)`) plus a `logger.info` line — this is how an owner/co-admin resolving another member's approval — native or linked — is attributed
7. 540s timeout — auto-rejects if no click
